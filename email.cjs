...
function wrap({ brandName, title, subtitle, body, logoCid }) {
  ...
  const header = logoCid
    ...
    : `<div style="color:#FFFFFF;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px">${brand}</div>`;
  return `<!DOCTYPE html>
    ...
    <tr><td align="center">
      <table width="580" cellpadding="0" cellspacing="0" style="background:white;border-radius:16px;overflow:hidden;border:1px solid #E2EAF4;box-shadow:0 4px 24px rgba(30,80,155,0.08)">
        <tr><td style="background:linear-gradient(135deg,#00265E,#1E509B);padding:32px 40px;text-align:center">
          ${header}
          <h1 style="color:#FFFFFF;margin:0;font-size:20px;font-weight:800;letter-spacing:-0.3px">${esc(title)}</h1>
          ${subtitle ? `<p style="color:#FFFFFF;margin:8px 0 0;font-size:13px">${esc(subtitle)}</p>` : ""}
        </td></tr>
